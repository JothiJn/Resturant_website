...
export function ReservationList({ initialReservations = [] }: { initialReservations?: DBReservation[] }) {
  ...
  const adaptedReservations: Reservation[] = initialReservations.map(r => ({
    id: r.id,
    ...
    barcode: `||| | ||| ${r.id.substring(0, 8)} || |`
  }));

  const [reservations, setReservations] = useState<Reservation[]>(adaptedReservations);
